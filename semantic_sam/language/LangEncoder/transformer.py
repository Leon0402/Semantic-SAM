...
from semantic_sam.utils.model import register_norm_module
...
class Transformer(nn.Module):
    ...
    def load_pretrained(self,
        ...
        if os.path.isfile(pretrained):
            ...
            stripped_key = lambda x: x[13:] if x.startswith('lang_encoder.'
                                                            ) else x
            pretrained_dict = {
                stripped_key(k): v
            ...
            for k, v in pretrained_dict.items():
                ...
                if need_init:
                    ...
                    if 'positional_embedding' in k and v.size(
                    ) != model_dict[k].size():
                        ...
                        if nH1 != nH2:
                            ...
                            if L1 != L2:
                                ...
                                posemb_grid = posemb.unsqueeze(dim=0).permute(
                                    0, 2, 1)
...
def lang_encoder(config_encoder, tokenizer, verbose, **kwargs):
    transformer = Transformer(context_length=config_encoder['CONTEXT_LENGTH'],
                              vocab_size=tokenizer.vocab_size,
                              width=config_encoder['WIDTH'],
                              layers=config_encoder['LAYERS'],
                              heads=config_encoder['HEADS'],
                              autogressive=config_encoder.get(
                                  'AUTOGRESSIVE', True))
