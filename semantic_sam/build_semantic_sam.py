...
def build_semantic_sam(model_type, ckpt):
    ...
    model_semantic_sam = BaseModel(
        opt, build_model(opt)).from_pretrained(ckpt).eval().cuda()
...
def plot_results(outputs, image_ori, save_path='../vis/'):
    ...
    if os.path.isdir(save_path):
        ...
        image_ori_name = os.path.basename(save_path).split(
            '.')[0] + '_input.png'
...
def plot_multi_results(iou_sort_masks,
                       area_sort_masks,
                       image_ori,
                       save_path='../vis/'):
    """
    ...
    create_long_image(iou_sort_masks).save(
        '../vis/all_results_sort_by_iou.png')
    create_long_image(area_sort_masks).save(
        '../vis/all_results_sort_by_areas.png')
