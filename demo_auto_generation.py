...
def inference(image, level=[0], *args, **kwargs):
    ...
    with torch.autocast(device_type='cuda', dtype=torch.float16):
        semantic = False
        model = model_sam
        a = interactive_infer_image_idino_m2m_auto(model, image, level, text,
                                                   text_part, text_thresh,
                                                   text_size, hole_scale,
                                                   island_scale, semantic,
                                                   *args, **kwargs)
        return a
...
text_model_level = gr.components.Textbox(label="Output levels",
                                         value="0,1,2,3,4,5",
                                         visible=True)
text_model_select = gr.components.Radio(
    ...
    value='All Prompt',
    label=
    "Our model learns 6 granularity prompts. [1-6] indicates output granularity from largest to smallest using different prompts.  'All prompt' means using all 6 granularity prompts."
)
image_out = gr.components.Image(label="Auto generation",
                                type="pil",
                                brush_radius=15.0).style(height=512)
...
with demo:
    with gr.Row():
        with gr.Column(scale=9.0):
            ...
            example1 = gr.Examples(
                ...
                inputs=image,
                label=
                'Example output of using different prompts for one image, output masks are from semantic, instance, to part level',
    ...
    runBtn.click(inference,
                 inputs=[image, text_model_select],
                 outputs=image_out)
