...
from semantic_sam.utils.constants import SCAN_37, SCAN_40, SCAN_20
...
def load_scannet_instances(name: str, dirname: str, split: str,
    ...
    for field in fileids:
        ...
        semseg_dir = image_dir.replace('color', name2folder[name]).replace(
            'jpg', 'png')
        r = {
            "file_name":
            image_dir,
...
def register_scannet_context(name, dirname, split, class_names=name2class):
    DatasetCatalog.register(
        name,
        lambda: load_scannet_instances(name, dirname, split, class_names))
    MetadataCatalog.get(name).set(stuff_classes=class_names[name],
                                  dirname=dirname,
                                  split=split,
                                  ignore_label=[0],
