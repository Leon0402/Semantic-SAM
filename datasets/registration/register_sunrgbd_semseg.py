...
from semantic_sam.utils.constants import SUN_RGBD_37

__all__ = ["load_sunrgbd_instances", "register_sunrgbd_context"]


def load_sunrgbd_instances(name: str, dirname: str, split: str,
    ...
    image_pths = sorted(
        glob.glob(os.path.join(dirname, 'image', split, '*.jpg')))
    semseg_pths = sorted(
        glob.glob(os.path.join(dirname, 'label37', split, '*.png')))
...
def register_sun_context(name, dirname, split, class_names=SUN_RGBD_37):
    DatasetCatalog.register(
        name,
        lambda: load_sunrgbd_instances(name, dirname, split, class_names))
    MetadataCatalog.get(name).set(stuff_classes=class_names,
                                  dirname=dirname,
                                  split=split,
                                  ignore_label=[0],
